...
class SchedulingData(LSSTObject):
    ...
    def __init__(self, configFile, surveyStartTime, surveyEndTime, astroSky, lsstDB, sessionID):
        ...
        for line in pairs:
            storeParam(lsstDB, sessionID, 0, 'schedulingData', line['index'], line['key'], line['val'])
    ...
    def initSurvey(self, startTime, endTime):
        ...
        if t < sunRise:
            ...
            (yesterday_sunRise, yesterday_sunSet, yesterday_sunRiseMJD,
                yesterday_sunSetMJD, yesterday_sunRiseTwil, yesterday_sunSetTwil) = x
            ...
            #   sunRise    t            sunSet          t
            x = self.sky.getIntTwilightSunriseSunset(t + DAY)
            (tomorrow_sunRise, tomorrow_sunSet, tomorrow_sunRiseMJD,
                tomorrow_sunSetMJD, tomorrow_sunRiseTwil, tomorrow_sunSetTwil) = x
    ...
    def updateLookAheadWindow(self):
        ...
        print("CLEANING DATA alt=%i size=%i  visible=%i size=%i" % (len_alt, siz_alt, len_visible,
                                                                    siz_visible))
        ...
        for t in self.visible.keys():
            ...
            for field in self.visible[t].keys():
                ...
                for prop in self.visible[t][field].keys():
                    len_visible += len(self.visible[t][field][prop])
                    siz_visible += sys.getsizeof(self.visible[t][field][prop])
        print("CLEANED  DATA alt=%i size=%i  visible=%i size=%i" % (len_alt, siz_alt, len_visible,
                                                                    siz_visible))
    ...
    def updateTargets(self, dictOfNewFields, propID, dateProfile, maxAirmass, dictFilterMinBrig,
                      dictFilterMaxBrig):
    ...
    def startNight(self, dateProfile):
        ...
        for propID in self.list_propID:
            ...
            self.computeTargetData(nextNight, dictOfNewFields, propID, maxAirmass, dictFilterMinBrig,
                                   dictFilterMaxBrig)
    ...
    def computeTargetData(self, initNight, dictOfNewFields, propID, maxAirmass, dictFilterMinBrig,
                          dictFilterMaxBrig):
        ...
        for t in self.visible.keys():
            ...
            for field in self.visible[t].keys():
                ...
                for prop in self.visible[t][field].keys():
                    len_visible += len(self.visible[t][field][prop])
                    siz_visible += sys.getsizeof(self.visible[t][field][prop])
        print("UPDATED  DATA alt=%i size=%i  visible=%i size=%i" % (len_alt, siz_alt, len_visible,
                                                                    siz_visible))
...
if (__name__ == '__main__'):
    ...
    sky = AstronomicalSky(None, obsProfile, 0, 0, None, "../conf/system/AstronomicalSky.conf", False,
                          './AstronomicalSky.log', 0)
