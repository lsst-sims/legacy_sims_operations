...
class Simulator(object):
    def __init__(self, lsstDB, obsProfile, sessionID, nRun, seeingEpoch, simStartDay, fov, idleDelay,
        ...
        self.done = False
        (self.latitude_RAD, self.longitude_RAD, self.height, self.simEpoch,
         self.pressure, self.temperature, self.relHumidity) = obsProfile
        ...
        # store config to DB
        for line in pairs:
            storeParam(self.lsstDB, sessionID, 0, schedDownConf, line['index'], line['key'], line['val'])
        ...
        # store config to DB
        for line in pairs:
            storeParam(self.lsstDB, sessionID, 0, unschedDownConf, line['index'], line['key'], line['val'])
    ...
    def start(self):
        ...
        (sunRise, sunSet, sunRiseMJD, sunSetMJD, sunRiseTwil,
            sunSetTwil) = self.sky.getTwilightSunriseSunset(t)
        ...
        if t > sunRise and t < sunSet:    # Middle of the day
            ...
            (sunRise, sunSet, sunRiseMJD, sunSetMJD,
                sunRiseTwil, sunSetTwil) = self.sky.getTwilightSunriseSunset(t + DAY)
            ...
            # Update sunRise, sunSet to reflect the values for tomorrow
            (sunRise, sunSet, sunRiseMJD, sunSetMJD, sunRiseTwil,
                sunSetTwil) = self.sky.getTwilightSunriseSunset(t + DAY)
            ...
            # --*-----|----------------------|------------->
            #   t  sunSet                  sunRise        t
            (yesterday_sunRise, yesterday_sunSet, yesterday_sunRiseMJD, yesterday_sunSetMJD,
                yesterday_sunRiseTwil, yesterday_sunSetTwil) = self.sky.getTwilightSunriseSunset(t - DAY)
        ...
        while t < self.nRun * YEAR:
            ...
            # print "nightCnt = %d nextSchedDown = %d nextUnschedDown = %d" %\
            #     (self.nightCnt, self.nextSchedDown, self.nextUnschedDown)
            ...
            if self.nightCnt == self.nextSchedDown or self.nightCnt == self.nextUnschedDown:
                ...
                while daysDown > 0:
                    ...
                    tonightSunsetTwil = sunSetTwil
                    self.log.info("Simulator: time:%d floor sunSetMJD:%f floor sunRiseMJD:%f" %
                                  (t, math.floor(sunSetMJD), math.floor(sunRiseMJD)))
                    if math.floor(sunSetMJD) != math.floor(sunRiseMJD):
                        ...
                        (sunRise, sunSet, sunRiseMJD, sunSetMJD, sunRiseTwil,
                            sunSetTwil) = self.sky.getTwilightSunriseSunset(t + DAY)
                    midnight = (tonightSunsetTwil + sunRiseTwil) / 2
                    self.log.info('Simulator: start(): t = %d (should be sunRise) sunSet = %d '
                                  'downtime_days = %d' % (t, sunSet, daysDown - 1))
                    self.twilightProfile = (sunRiseTwil, tonightSunsetTwil)

                    daysDown -= 1

                # record entire downtime in one DB entry
                # sql = 'INSERT INTO %s VALUES (NULL, %d, "%s", %d, %d, %d, %d, %d, %f, "%s")' %\
                #     (self.dbTableDict['downHist'], self.sessionID, activityDown, duration, startDownTime, t,
                #         startDownNight, self.nightCnt, cloudiness, schedDown)
            ...
            # This has been commented out for discussion with Francisco
                ...
                while self.nextUnschedDown != -1 and self.nextUnschedDown < self.nightCnt:
                    ...
                    if self.nextUnschedDown + self.unschedDownDuration[0] <= self.nightCnt:
                        ...
                        self.log.info('Overlapping unscheduled downtime specified: partial = %d' %
                                      partialDown)
            ...
            # regular observing
                ...
                while (t < sunRise):
                    ...
                    if cloudiness > self.maxCloud:
                        if self.log:
                            self.log.info("Simulator: Cloudy above %0.3f (date:%d), skip time slice" %
                                          (self.maxCloud, t))
                        self.telescope.Park()
                    ...
                    # Observe next field
                        ...
                        if oRank != 0:
                            ...
                            if self.log:
                                self.log.info('Simulator: observing: last event: %d; idle %d sec; '
                                              'slew %d sec; expose %d sec' % (t, idle, oSlew, oExp))
                        else:
                            # yield hold, self, self.idleDelay
                            t = t + self.idleDelay
                            #eventTime = self.idleDelay
                    ...
                    if t > tonightSunsetTwil and lastEvent < tonightSunsetTwil:
                        ...
                        # self.timeHist.add (self.sessionID, self.nightCnt, self.dateProfile, END_DUSK)
                    ...
                    # Check for dawn - MM
                        ...
                        # self.timeHist.add (self.sessionID, self.nightCnt, self.dateProfile, START_DAWN)
                        self.obsScheduler.flushSkyCache()
    ...
    def initMoonPhase(self, date):
        ...
        print "MoonPhasing: past:%f present:%f future:%f trendtoFull:%s" %\
            (previousDayPhase, currentPhase, nextDayPhase, self.moonTrendToFull)
...
#        return(moonRA_RAD,moonDec_RAD,moonPhase_PERCENT)
    ...
    def startNight(self, date, midnight, sunRise):
        ...
        if self.moonTrendToFull:  # i.e. previous moon was waxing
            if self.lastNightPhase > moonPhase_PERCENT:
                ...
                # self.timeHist.add (self.sessionID, self.nightCnt, self.dateProfile, MOON_WANING)
                if self.lunationCount > 0 and self.lunationCount % 12 == 0:
                    ...
                    # self.timeHist.add (self.sessionID, self.nightCnt, self.dateProfile, NEW_YEAR)
                self.lunationCount += 1
                self.moonTrendToFull = False
                self.log.info("New Lunation: Moon phase switching to waning: date:%d lastphase:%f "
                              "newphase:%f startNewLunation:%d" % (date, self.lastNightPhase,
                                                                   moonPhase_PERCENT, startNewLunation))
            ...
            if moonPhase_PERCENT > self.lastNightPhase:
                ...
                # self.timeHist.add (self.sessionID, self.nightCnt, self.dateProfile, MOON_WAXING)
                self.log.info("Moon phase switching to waxing: date:%d lastphase:%f newphase:%f "
                              "startNewLunation:%d" % (date, self.lastNightPhase, moonPhase_PERCENT,
                                                       startNewLunation))
        ...
        # self.timeHist.add(self.sessionID, self.nightCnt, self.dateProfile, START_NIGHT)

        # Finally:  let ObsScheduler know about the startNight
        self.obsScheduler.startNight(self.dateProfile, self.moonProfile, startNewLunation, startNewYear,
                                     self.fov, self.nRun, self.nightCnt)

        (sunRiseTwil, tonightSunsetTwil) = self.twilightProfile
        if self.log:
            # self.log.info('Simulator: NightStart=%d t=%d tonightSunsetTwil=%d Midnight=%d sunRiseTwil=%d '
            #               'sunRise=%d' % (now(), date, tonightSunsetTwil, midnight, sunRiseTwil, sunRise))
            self.log.info('Simulator: NightStart t=%d tonightSunsetTwil=%d Midnight=%d sunRiseTwil=%d '
                          'sunRise=%d' % (date, tonightSunsetTwil, midnight, sunRiseTwil, sunRise))

        return

    def startDay(self, date):
        """
        Perform any necessary setup for the end of the night, triggering filters swap for now.
        """
        self.dateProfile = self.sky.computeDateProfile(date)
        (date, MJD, lst_RAD) = self.dateProfile
        self.lsstDB.addTimeHistory(self.sessionID, date, MJD, self.nightCnt, END_NIGHT)
        # self.timeHist.add (self.sessionID, self.nightCnt, self.dateProfile,END_NIGHT)
    ...
    def setupSimulation(self):
        ...
        self.telescope = Instrument(lsstDB=self.lsstDB, sessionID=self.sessionID,
                                    dbTableDict=self.dbTableDict, obsProfile=self.obsProfile,
                                    instrumentConf=self.instrumentConf, log=self.log, logfile=self.logfile,
        ...
        self.schedulingData = SchedulingData(self.schedulingDataConf, 0, self.nRun * YEAR, self.sky,
                                             self.lsstDB, self.sessionID)

        # Init the Weather Simulator
        self.weather = Weather(lsstDB=self.lsstDB, date=0., dbTableDict=self.dbTableDict,
                               seeingEpoch=self.seeingEpoch, simStartDay=self.simStartDay, log=self.log,
                               logfile=self.logfile, verbose=self.verbose)
        ...
        self.obsScheduler = ObsScheduler(lsstDB=self.lsstDB, schedulingData=self.schedulingData,
                                         obsProfile=self.obsProfile, dbTableDict=self.dbTableDict,
                                         telescope=self.telescope, weather=self.weather, sky=self.sky,
                                         filters=self.filters, sessionID=self.sessionID,
                                         runSeeingFudge=self.runSeeingFudge, schedulerConf=self.schedulerConf,
                                         log=self.log, logfile=self.logfile, verbose=self.verbose)

        # Create a TAC object and activate it.
        self.tac = TAC(lsstDB=self.lsstDB, nRun=self.nRun, schedulingData=self.schedulingData, sky=self.sky,
                       weather=self.weather, obsScheduler=self.obsScheduler, sessionID=self.sessionID,
                       fov=self.fov, filters=self.filters, nearEarthConf=self.nearEarthConf,
                       weakLensConf=self.weakLensConf, superNovaConf=self.superNovaConf,
                       superNovaSubSeqConf=self.superNovaSubSeqConf, kuiperBeltConf=self.kuiperBeltConf,
                       WLpropConf=self.WLpropConf, targetList=self.targetList, dbTableDict=self.dbTableDict,
                       log=self.log, logfile=self.logfile, verbose=self.verbose)
        return
