...
class InstrumentPosition(object):
    ...
    def __init__(self, ra_RAD=0.0, dec_RAD=0.0, angle_RAD=0.0, filter='r', exptime=0.0, time=0.0,
                 alt_RAD=1.553343, az_RAD=0.0, pa_RAD=0.0):
    ...
    def Set(self, ra_RAD, dec_RAD, angle_RAD, filter, exptime, date, alt_RAD, az_RAD, pa_RAD):
    ...
    def Copy(self, newpos):
        ...
        self.Set(newpos.RA_RAD, newpos.DEC_RAD, newpos.ANG_RAD, newpos.Filter_Pos, newpos.Exposure_Time,
...
class InstrumentState(InstrumentPosition):
    ...
    def __init__(self, alt_RAD=1.553343, az_RAD=0.0, pa_RAD=0.0, filter='r', exptime=0.0, ra=0.0, dec=0.0,
        ...
        super(InstrumentState, self).__init__(ra * DEG2RAD, dec * DEG2RAD, 0.0, filter, exptime, time,
                                              alt_RAD, az_RAD, pa_RAD)

        (self.longitude_RAD, self.latitude_RAD, self.height, self.simEpoch, d1, d2, d3) = obsProfile

        if config_dict is None:
            if instrumentConf is None:
    ...
    def GetShortestDistanceWithWrap(self, target_RAD, current_abs_RAD, min_abs_RAD=None, max_abs_RAD=None):
        ...
        if min_abs_RAD is not None:
            ...
            # if the target angle is unreachable then sets an arbitrary value
    ...
    def GetTelAzDistanceWithWrap(self, az_RAD):
        ...
        return self.GetShortestDistanceWithWrap(az_RAD, self.TelAz_Pos_RAD, self.TelAz_MinPos_RAD,
                                                self.TelAz_MaxPos_RAD)

    def GetRotatorDistanceWithWrap(self, newangle_RAD):

        return self.GetShortestDistanceWithWrap(newangle_RAD, self.Rotator_Pos_RAD, self.Rotator_MinPos_RAD,
                                                self.Rotator_MaxPos_RAD)
...
class InstrumentSlewParams (object):
    ...
    def __init__(self, config_dict, instrumentConf=None):
        ...
        self.activities = ["DomAlt", "DomAz", "TelAlt", "TelAz", "TelOpticsOL", "TelOpticsCL", "Rotator",
                           "Filter", "ADC", "InsOptics", "GuiderPos", "GuiderAdq", "Settle", "DomSettle",
                           "Exposure", "Readout"]
...
#        Simulation.Process.__init__ (self)
        ...
        self.params = InstrumentParams(latitude_RAD, longitude_RAD, height)
        self.current_state = InstrumentState(config_dict=self.config_dict, instrumentConf=instrumentConf,
                                             obsProfile=obsProfile)
        self.target_state = InstrumentState(config_dict=self.config_dict, instrumentConf=instrumentConf,
                                            obsProfile=obsProfile)
        self.park_state = InstrumentState(config_dict=self.config_dict, instrumentConf=instrumentConf,
                                          obsProfile=obsProfile)
        self.next_state = InstrumentState(config_dict=self.config_dict, instrumentConf=instrumentConf,
                                          obsProfile=obsProfile)
        self.slew_params = InstrumentSlewParams(config_dict=self.config_dict, instrumentConf=instrumentConf)
    ...
    def GetDelayFor_DomAz(self, target_state, prev_state):
        ...
        # buffer_RAD = self.GetSlitBuffer(target_state.DomAlt_Pos_RAD, prev_state.DomAlt_Pos_RAD)
        buffer_RAD = 0.0
    ...
    def GetDelayFor_Settle(self, target_state, prev_state):
        if target_state.TelAlt_Pos_RAD == prev_state.TelAlt_Pos_RAD and \
                target_state.TelAz_Pos_RAD == prev_state.TelAz_Pos_RAD:
            return 0.0
        else:
...
#
    ...
    def GetSlewDelay(self, target_position, prev_state, allSlewData=False):
        ...
        if prev_state.Tracking is False:
    ...
    def Slew(self, new_position):
        ...
        if new_position.Filter_Pos != self.current_state.Filter_Pos:
            ...
            if self.log:
                bn = min(int(self.slew_params.Filter_MaxChangesBurstNumber),
                         len(self.filterChangesTimeHistory) - 1)
                if bn > 0:
                    bt = self.filterChangesTimeHistory[-1] - self.filterChangesTimeHistory[-1 - bn]
                else:
                    bt = 0
                an = min(int(self.slew_params.Filter_MaxChangesAvgNumber),
                         len(self.filterChangesTimeHistory) - 1)
                if an > 0:
    ...
    def GetDelayForTarget(self, ra_RAD, dec_RAD, dateProfile, exposureTime, filter):
        """
        ...
        (az_RAD, d1, d2, alt_RAD, d4, d5, pa_RAD, d7, d8) = pal.altaz(ha_RAD, dec_RAD,
                                                                      self.params.latitude_RAD)
    ...
    def Observe(self, ra_RAD, dec_RAD, dateProfile, exposureTime, filter, delay, log=False):
        """
        ...
        ha_RAD = lst_RAD - ra_RAD
        # (az_RAD, d1, d2, alt_RAD, d4, d5, pa_RAD, d7, d8) = slalib.sla_altaz(ha_RAD, dec_RAD,
        #                                                                      self.params.latitude_RAD)
        (az_RAD, d1, d2, alt_RAD, d4, d5, pa_RAD, d7, d8) = pal.altaz(ha_RAD, dec_RAD,
                                                                      self.params.latitude_RAD)
...
#        sql += '%f )' % (delay)
        ...
        for activity in self.lastslew_delays.keys():
            if activity in self.lastslew_criticalpath:
                ...
                cp = 'False'
            slewactivity = activitySlew(activity, self.lastslew_delays[activity], cp)
...
#            sql += '"%s" )' % (cp)
    ...
    def DBrecordState(self, state, initfinal):
        ...
        slewState = stateSlew(state.TIME, state.RA_RAD, state.DEC_RAD, tracking, state.ALT_RAD, state.AZ_RAD,
                              state.PA_RAD, state.DomAlt_Pos_RAD, state.DomAz_Pos_RAD, state.TelAlt_Pos_RAD,
                              state.TelAz_Pos_RAD, state.Rotator_Pos_RAD, state.Filter_Pos, initfinal)
...
#	return n
    ...
    def DBrecordMaximumSpeeds(self):
        ...
        slewSpeeds = speedsSlew(self.current_state.DomAlt_Spd_RDS, self.current_state.DomAz_Spd_RDS,
                                self.current_state.TelAlt_Spd_RDS, self.current_state.TelAz_Spd_RDS,
