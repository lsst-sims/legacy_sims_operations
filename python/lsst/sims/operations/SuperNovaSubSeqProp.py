...
class SuperNovaSubSeqProp (TransSubSeqProp):
    """
    ...
    def __init__ (self, 
		  ...
		  sky, 
                  ...
                  dbTableDict=None,
                  log=False,
                  logfile='./SuperNovaSubSeqProp.log',
                  verbose=0,
                  superNovaSubSeqConf=DefaultSNSSConfigFile):
        """
        ...
        """
        super (SuperNovaSubSeqProp, self).__init__ (lsstDB=lsstDB,
					      propConf=superNovaSubSeqConf,
                                              propName='SNSS',
					      propFullName='SuperNovaSubSeq',
                                              sky=sky, 
                                              weather=weather,
                                              sessionID=sessionID,
                                              filters=filters,
                                              ...
                                              logfile=logfile,
                                              verbose=verbose,
					      transientConf=superNovaSubSeqConf)
	...
	self.log.info('proposal configfile=%s subsequences:' % (str(superNovaSubSeqConf)))
	self.log.info('         SubSeqName=%s' % (str(self.subSeqName)))
        ...
        #original SNSS read config fields
        try:
        ...
        # Save the SNSS configuration parameters to the DB
        for line in pairs:
            storeParam (self.lsstDB, self.sessionID, self.propID, 'snss', line['index'],
                        line['key'], line['val'])
	...
	self.TransientType = 'SuperNovaSubSeq'
...
#
#        super (SuperNovaSubSeqProp, self).startNight (dateProfile,moonProfile,startNewLunation,randomizeSequencesSelection,nRun)
#
#        return
    ...
    def updateTargetList (self, dateProfile, obsProfile, sky, fov):
       ...
       #RAA Following overkill for simulation; needed for Polar sites
       ...
       #print (sql)
       print ('*** Found %d SNSS fields for propID=%d***' % (len (res),self.propID))
       ...
       print ('*** Found %d SNSS fields for propID=%d for new sequences ***' % (len (res),self.propID))
