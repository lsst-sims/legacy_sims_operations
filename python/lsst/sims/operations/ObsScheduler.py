...
class ObsScheduler(LSSTObject):
    ...
    def startNight(self, dateProfile, moonProfile, startNewLunation, startNewYear, fov, nRun, nightCnt):
        """
        ...
        for proposal in self.proposals_list:
            ...
            # print "ObsScheduler.startNight: prop %s prop.nextNight = %d self.nightCnt = %d "\
            #     "prop.hiatus = %d" % (proposal.propFullName, proposal.nextNight, self.nightCnt,
            #                           proposal.hiatusNights)
            if proposal.IsActiveTonight(date, self.nightCnt) is False:
                continue

            # Build new target list
            fields = proposal.updateTargetList(dateProfile, self.obsProfile, self.sky, fov)
            self.targets.update(fields)
            print "Targetcount:%d" % len(self.targets)

            # Notify all proposals of startNight processing
            proposal.startNight(dateProfile, moonProfile, startNewLunation, self.randomizeSequencesSelection,
                                nRun, mountedFiltersList)
            if startNewYear:
    ...
    def SwapExtraFilterIn(self):
        ...
        # for proposal in self.interProposalRank.keys():
        for proposal in self.proposals_list:
            if proposal.IsActive(date, self.nightCnt) is False:
                continue
...
#        return
    ...
    def suggestObservation(self, dateProfile, moonProfile, twilightProfile, cloudiness):
        """
        ...
        if self.reuseRanking <= 0:
            ...
            if self.recalcSky <= 0:
                ...
                if seeing < self.tooGoodSeeingLimit:
                    if self.log:
                        self.log.info("obsScheduler:suggestObservation: seeing (%f) too good, reset to %f "
                                      "date:%d." % (seeing, self.tooGoodSeeingLimit, date))
            ...
            for proposal in self.proposals_list:
                ...
                targetObs = proposal.suggestObs(self.dateProfile, self.numSuggObsPerProp, self.exclusiveObs,
                                                self.minDistance2Moon, self.rawSeeing, self.seeing,
                                                self.transparency, sdnight, sdtime)
                if not targetObs:
                    continue
...
#          self.log.info("totPotentialTargets = %d" % totPotentialTargets)
        ...
        for fieldID in fields:
            ...
            for key in keyList:
                ...
                # Compute slew time
                slewTime = self.telescope.GetDelayForTarget(ra_RAD=ra * DEG2RAD, dec_RAD=dec * DEG2RAD,
                                                            dateProfile=self.dateProfile,
                                                            exposureTime=expTime, filter=filter)
        ...
        if maxrank > 0:
            ...
            self.winner = Observation(ra=win_ra, dec=win_dec, fieldID=win_fieldID, filter=win_filter,
                                      slewTime=win_slewTime, exposureTime=win_exposureTime,
                                      exclusiveBlockRequired=(win_propXblk is not None), propID=win_propXblk,
                                      dateProfile=self.dateProfile, moonProfile=self.moonProfile)
...
#                 moonAlt_RAD,azimuth_RAD)
    ...
    def closeObservation(self, winner):
        ...
        """
        # Move the telescope
        (delay, rotatorSkyPos_RAD, rotatorTelPos_RAD, alt_RAD, az_RAD, slewdata,
            slewinitstate, slewfinalstate, slewmaxspeeds,
        ...
        if delay != winner.slewTime:
            print "WARNING: ObsScheduler.closeObservation delay:%f != winner.slewTime:%f" %\
                (delay, winner.slewTime)
        ...
        (winner.moonRA_RAD, winner.moonDec_RAD, winner.moonPhase, winner.moonAlt,
            winner.moonAz) = moonProfileAltAz
        moonProfile = (winner.moonRA_RAD, winner.moonDec_RAD, winner.moonPhase)

        (skyBright, distance2moon, moonAlt_RAD,
            brightProfile) = self.sky.getSkyBrightness(winner.fieldID, winner.ra, winner.dec, winner.altitude,
                                                       dateProfile, moonProfile, self.twilightProfile)
        winner.skyBrightness = skyBright
        winner.distance2moon = distance2moon
        (winner.phaseAngle, winner.extinction, winner.rScatter, winner.mieScatter, winner.moonIllum,
        ...
        self.log.info("    finRank=%f airmass=%f brightness=%f filtBright=%f rawSeeing=%f seeing=%f" %
                      (winner.finRank, winner.airmass, winner.skyBrightness, winner.filterSkyBright,
                       winner.rawSeeing, winner.seeing))
        self.log.info("    alt=%f az=%f pa=%f moonRA=%f moonDec=%f moonPh=%f dist2moon=%f transp=%f" %
                      (winner.altitude, winner.azimuth, winner.parallactic, winner.moonRA_RAD,
                       winner.moonDec_RAD, winner.moonPhase, winner.distance2moon, winner.transparency))
        self.log.info("    solarE=%f sunAlt=%f sunAz=%f moonAlt=%f moonAz=%f moonBright=%f darkBright=%f" %
        ...
        obsHist = self.lsstDB.addObservation(slewdata.slewCount, winner.filter, winner.date, winner.mjd,
                                             ...
                                             winner.airmass, winner.skyBrightness, winner.filterSkyBright,
                                             winner.rotatorSkyPos, winner.lst, winner.altitude,
                                             winner.azimuth, winner.distance2moon, winner.solarElong,
                                             winner.moonRA_RAD, winner.moonDec_RAD, winner.moonAlt,
                                             winner.moonAz, winner.moonPhase, winner.sunAlt, winner.sunAz,
        ...
        self.lsstDB.addSlewState(slewinitstate.slewStateDate, slewinitstate.tra, slewinitstate.tdec,
                                 slewinitstate.tracking, slewinitstate.alt, slewinitstate.az,
                                 slewinitstate.pa, slewinitstate.DomAlt, slewinitstate.DomAz,
                                 slewinitstate.TelAlt, slewinitstate.TelAz, slewinitstate.RotTelPos,
                                 slewinitstate.Filter, slewinitstate.state, slewHist.slewID)
        self.lsstDB.addSlewState(slewfinalstate.slewStateDate, slewfinalstate.tra, slewfinalstate.tdec,
                                 slewfinalstate.tracking, slewfinalstate.alt, slewfinalstate.az,
                                 slewfinalstate.pa, slewfinalstate.DomAlt, slewfinalstate.DomAz,
                                 slewfinalstate.TelAlt, slewfinalstate.TelAz, slewfinalstate.RotTelPos,
                                 slewfinalstate.Filter, slewfinalstate.state, slewHist.slewID)

        self.lsstDB.addSlewMaxSpeeds(slewmaxspeeds.DomAltSpd, slewmaxspeeds.DomAzSpd, slewmaxspeeds.TelAltSpd,
                                     slewmaxspeeds.TelAzSpd, slewmaxspeeds.RotSpd, slewHist.slewID)
        ...
        for proposal in self.proposals_list:
            if proposal.IsActive(date, self.nightCnt):
                ...
                proposal.closeObservation(winner, obsHist.obsHistID, self.twilightProfile)

                # print "ObsScheduler.closeObs(): In proposal?: fieldID: %d date: %d propID: %d" % \
                #     (self.winner.fieldID, t, proposal.propID)
    ...
    def addProposal(self, proposal, rank=0):
        ...
        if not isinstance(proposal, Proposal):
            raise (ParamTypeError, 'proposal needs to be an instance of the Proposal class')
        ...
        if proposal.ComplyToFilterChangeBurstConstraint(filterBurstNumber, filterBurstTime, visitTime,
