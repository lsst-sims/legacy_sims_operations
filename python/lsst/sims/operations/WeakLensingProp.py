...
class WeakLensingProp(Proposal):
    ...
    def __init__(self, lsstDB, schedulingData, sky, weather, sessionID, filters, fov,
        ...
        super(WeakLensingProp, self).__init__(lsstDB=lsstDB, propConf=weakLensConf, propName='WL',
                                              propFullName='WeakLensing', sky=sky, weather=weather,
...
#            self.goalNVisits = 30.
        ...
        print 'GoalVisitsFieldFilter for propID=%d %s = %s' % (self.propID, self.propFullName,
                                                               str(self.GoalVisitsFieldFilter))
        ...
        # store config to DB
        for line in pairs:
            storeParam(self.lsstDB, self.sessionID, self.propID, 'weakLensing', line['index'], line['key'],
                       line['val'])
        ...
        # Setup the relative importance of each filter. For convenience we use a dictionary of the form
        # {filter: N} where N is the desired fraction of the total number of observations for that particular
        # filter. If all the filters were equally desirable, then N would be always equal to
    ...
    def startNight(self, dateProfile, moonProfile, startNewLunation, randomizeSequencesSelection, nRun,
        ...
        super(WeakLensingProp, self).startNight(dateProfile, moonProfile, startNewLunation,
                                                mountedFiltersList)
    ...
    def GetProgressPerFilter(self):
        ...
        for filter in filters:
            if self.GoalVisitsFieldFilter[filter] > 0:
                if len(allfields) > 0:
                    divisor = len(allfields) / self.GoalVisitsFieldFilter[filter]
                    progressFilter[filter] = float(coaddedFilterProgress[filter]) / divisor
                else:
                    # no fields observed for this filter
    ...
    def rankAreaDistribution(self, listOfFieldsToEvaluate, sdnight, sdtime, dateProfile, rawSeeing, seeing,
                             transparency):
        ...
        if needTonight > 0:
            ...
            GlobalNeedFactor = (self.maxNeedAfterOverflow / (-needTonight + 1)) / self.GoalVisitsTonight
        ...
        for fieldID in listOfFieldsToEvaluate:
            ...
            if airmass > self.maxAirmass:
                fields_invisible += 1
                if self.log and self.verbose > 1:
                    self.log.info('TOSS: propID=%d field=%d  WeakLensingProp: suggestObs(): too low:%f' %
            ...
            if self.ProgressToStartBoost < progress_avg < 1.0:
                progAvMinusBoost = progress_avg - self.ProgressToStartBoost
                oneMinusBoost = 1.0 - self.ProgressToStartBoost
                FieldNeedFactor += self.MaxBoostToComplete * progAvMinusBoost / oneMinusBoost
            ...
            for filter in allowedFilterList:
                ...
                if filterSeeingList[filter] > self.FilterMaxSeeing[filter]:
                    ffilter_badseeing += 1
                    if self.log and self.verbose > 1:
                        self.log.info('TOSS: propID=%d field=%d  filter=%s  WeakLensingProp: suggestObs(): '
                                      'bad seeing:%f' % (self.propID, fieldID, filter,
                ...
                if GlobalNeedFactor > 0.0:
                    if FieldNeedFactor > 0.0:
                        ...
                        visitsMinusGoal = nVisits[filter] - self.GoalVisitsFieldFilter[filter] + 1
                        numerator = self.maxNeedAfterOverflow / visitsMinusGoal
                        FilterNeedFactor = numerator / self.GoalVisitsFieldFilter[filter]
        ...
        if self.log and self.verbose > 0:
            self.log.info('%s: rankAreaDistribution propID=%d : Fields received=%i invisible=%i moon=%i '
                          'Field-Filters allowed=%i badseeing=%i proposed=%i' % (self.propConf, self.propID,
                                                                                 fields_received,
                                                                                 fields_invisible,
                                                                                 fields_moon, ffilter_allowed,
                                                                                 ffilter_badseeing,
                                                                                 ffilter_proposed))

        return

    def rankAreaDistributionWithLookAhead(self, listOfFieldsToEvaluate, sdnight, sdtime, dateProfile,
                                          acoshrawSeeing, seeing, transparency):
        ...
        for field in listOfFieldsToEvaluate:
            ...
            if field == self.last_observed_fieldID and self.last_observed_wasForThisProposal and \
                    not self.AcceptConsecutiveObs:
                fieldNeed = -1
                fieldTime = -1
                continue
            ...
            if fieldTime == 0:
                ...
                self.log.info('%s: rankAreaDistributionWithLookAhead propID=%d field=%d removed invisible' %
                              (self.propConf, self.propID, field))
            elif fieldNeed == 0:
                # the field for all its filters is no longer needed
                fields_complete += 1
                del self.targets[field]
                self.log.info('%s: rankAreaDistributionWithLookAhead propID=%d field=%d removed complete' %
                              (self.propConf, self.propID, field))
        ...
        if self.log and self.verbose > 0:
            self.log.info('%s: rankAreaDistributionWithLookAhead propID=%d : Fields received=%i complete=%i '
                          'invisible=%i moon=%i badseeing=%i notime=%i noneed=%i proposed=%i' %
                          (self.propConf, self.propID, fields_received, fields_complete, fields_invisible,
                           fields_moon, ffilter_badseeing, ffilter_notime, ffilter_noneed, ffilter_proposed))

        return

    def suggestObs(self, dateProfile, n=1, exclusiveObservation=None, minDistance2Moon=0.0, rawSeeing=0.0,
    ...
    def closeObservation(self, observation, obsHistID, twilightProfile):
        ...
        if obs is not None:
            ...
            if self.log and self.verbose > 0:
                ...
                self.log.info('WeakLensingProp: closeObservation() propID=%d field=%d filter=%s '
                              'propRank=%.4f finRank=%.4f t=%dd%02dh%02dm%02ds progress=%d%%' %
                              (self.propID, obs.fieldID, obs.filter, obs.propRank, obs.finRank, t_days,
                               t_hour, t_mins, t_secs, int(100 * progress)))
    ...
    def updateTargetList(self, dateProfile, obsProfile, sky, fov):
        ...
        import Sun
        s = Sun.Sun()
        (sunRise, sunSet) = s.__sunriset__(yy, mm, dd, lon_RAD * RAD2DEG, lat_RAD * RAD2DEG,
        ...
        sql += 'WHERE %s BETWEEN %f AND %f AND ' % (dbFov, fovEpsilon1, fovEpsilon2)
        ...
        self.schedulingData.updateTargets(fields, self.propID, dateProfile, self.maxAirmass,
                                          self.FilterMinBrig, self.FilterMaxBrig)
