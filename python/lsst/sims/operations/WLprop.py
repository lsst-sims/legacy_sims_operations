...
class WLprop (TransSubSeqProp):
    ...
    def __init__(self, lsstDB, schedulingData, sky, weather, sessionID, filters, targetList=None,
        ...
        """
        super(WLprop, self).__init__(lsstDB=lsstDB, propConf=WLpropConf, propName='WLTSS',
                                     propFullName='WeakLensingTSS', sky=sky, weather=weather,
                                     sessionID=sessionID, filters=filters, targetList=targetList,
                                     dbTableDict=dbTableDict, log=log, logfile=logfile, verbose=verbose,
        ...
        if self.WLtype and self.overflowLevel > 0.0:
            self.overflow = True
        else:
            self.overflow = False
...
#        return rankForFilters
    ...
    def updateTargetList(self, dateProfile, obsProfile, sky, fov):
        ...
        if raMaxNewSeq > raMinNewSeq:
            sqlNewSeq += '%s BETWEEN %f AND %f AND ' % (dbRA, raMinNewSeq, raMaxNewSeq)
        elif raMaxNewSeq < raMinNewSeq:
            sqlNewSeq += '(%s BETWEEN %f AND 360.0 OR ' % (dbRA, raMinNewSeq)
            sqlNewSeq += '%s BETWEEN 0.0 AND %f) AND ' % (dbRA, raMaxNewSeq)
        else:
            sqlNewSeq += '%s BETWEEN 0.0 AND 360.0 AND ' % (dbRA)

        if raAbsMax >= raAbsMax:
            sql += '%s BETWEEN %f AND %f AND ' % (dbRA, raAbsMin, raAbsMax)
            sqlNewSeq += '%s BETWEEN %f AND %f AND ' % (dbRA, raAbsMin, raAbsMax)
        else:
            sql += '(%s BETWEEN %f AND 360.0 OR ' % (dbRA, raAbsMin)
            sql += '%s BETWEEN 0.0 AND %f) AND ' % (dbRA, raAbsMax)
            sqlNewSeq += '(%s BETWEEN %f AND 360.0 OR ' % (dbRA, raAbsMin)
            sqlNewSeq += '%s BETWEEN 0.0 AND %f) AND ' % (dbRA, raAbsMax)

        latRadPlusMaxReach = (lat_RAD * RAD2DEG) + abs(self.maxReach)
        latRadMinusMaxReach = (lat_RAD * RAD2DEG) - abs(self.maxReach)
        sql += '%s BETWEEN %f AND %f order by FieldRA,FieldDec' % (dbDec, latRadMinusMaxReach,
                                                                   latRadPlusMaxReach)

        sqlNewSeq += '%s BETWEEN %f AND %f order by FieldRA,FieldDec' % (dbDec, latRadMinusMaxReach,
                                                                         latRadPlusMaxReach)
        ...
        #r1 = resident()
        #s1 = stacksize()
        # self.log.info("WL: updateTargetList:(entry:exit) mem: %d:%d resMem: %d:%d stack: %d:%d" % (m0, m1,
        #                                                                                            r0, r1,
        #                                                                                            s0, s1))
        # print("WL: updateTargetList:(entry:exit) mem: %d:%d resMem: %d:%d stack: %d:%d" % (m0, m1, r0, r1,
        #                                                                                    s0, s1))

        self.schedulingData.updateTargets(self.targets, self.propID, dateProfile, self.maxAirmass,
