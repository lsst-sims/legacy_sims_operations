...
class AstronomicalSky(LSSTObject):
    ...
    def getTwilightSunriseSunset(self, date):
        ...
        self.log.info("getTwilightSunriseSunset:date:%d mjd:%f sunriseMJD:%f sunsetMJD:%f sunriseDate:%f "
                      "sunsetDate:%f" % (date, mjd, sunRiseMJD, sunSetMJD, sunriseDate, sunsetDate))
        self.log.info("getTwilightSunriseSunset:date:%d mjd:%f sunriseTwilMJD:%f sunsetTwilMJD:%f "
                      "sunriseTwilDate:%f sunsetTwilDate:%f" % (date, mjd, sunRiseTwilMJD, sunSetTwilMJD,
    ...
    def getIntTwilightSunriseSunset(self, date):
        ...
        (sunriseDate, sunsetDate, sunRiseMJD, sunSetMJD, sunriseTwilDate,
            sunsetTwilDate) = self.getTwilightSunriseSunset(date)
        return (int(sunriseDate), int(sunsetDate), sunRiseMJD, sunSetMJD, int(sunriseTwilDate),
                int(sunsetTwilDate))
    ...
    def getNightMidpoint(self, date):
        ...
        self.log.info("getNightMidPoint:date:%d mjd:%f sunrise:%f sunset:%f sunriseMJD:%f sunsetMJD:%f "
                      "midpoint%f midpointMJD:%f" % (date, mjd, sunRise, sunSet, sunRiseMJD, sunSetMJD,
                                                     midpoint, midpointMJD))
        return midpoint
    ...
    def getPlanetPosition(self, planet, date):
        ...
        # (ra_RAD, dec_RAD, diam) = slalib.sla_rdplan(mjd, self.PLANETS[planet], self.longitude_RAD,
        #                                             self.latitude_RAD)
    ...
    def getSkyBrightness(self, fieldID, ra, dec, targetAlt_RAD, dateProfile, moonProfile, twilightProfile,
                         extinction=0.172, skyBrightness=21.587):
        """
        ...
        skyBr = skyBrightness * x(targetZD_DEG) * 10. ** (-0.4 * k * (x(targetZD_DEG) - 1.))
...
#            pass
        ...
        #                           dec * DEG2RAD,
        #                           self.latitude_RAD)
        (az_RAD, d1, d2, alt_RAD, d4, d5, pa_RAD, d7, d8) = pal.altaz(lha_RAD, dec * DEG2RAD,
                                                                      self.latitude_RAD)
        ...
        # print "t: %d raDEG:%f decDEG:%f mjd:%f lstRAD:%f lhaDEG:%f lha_RAD:%f altRAD:%f zdRAD:%f am:%f" %\
        #     (date, ra, dec, mjd, lst_RAD, lha, lha_RAD, alt_RAD, zd_RAD, am)
...
#        self.airmassCache[key] = (am,alt_RAD,az_RAD)
    ...
    def getHAforAirmass(self, airmass):
        """
        Given an airmass, this function calculates the corresponding HA in the west, in HOUR units.
        """
    ...
    def altitude(self, latitude_RAD, dec_RAD, hourAngle_RAD):
        ...
        #                           dec_RAD,
        #                           latitude_RAD)
        (az_RAD, d1, d2, alt_RAD, d4, d5, d6, d7, d8) = pal.altaz(hourAngle_RAD, dec_RAD, latitude_RAD)
...
if (__name__ == '__main__'):
    ...
    class KnownValues(unittest.TestCase):
        ...
        def testGetPlanetPosition(self):
            ...
            sky = AstronomicalSky(self.ctioLat, self.ctioLon, self.ctioHeight, 0., self.ctioEpoch)
        ...
        def testGetDistance(self):
            ...
            for (input, expectedResult) in self.knownDistanceValues:
                ...
                self.assertEqual('%.04f' % (result), '%.04f' % (expectedResult))
            return
        ...
        def testGetSkyBrightness(self):
            ...
            for (input, expectedResult) in self.knownSkyBrightness:
                ...
                result = sky.getSkyBrightness(target=t, date=d, extinction=k, skyBrightness=s)
