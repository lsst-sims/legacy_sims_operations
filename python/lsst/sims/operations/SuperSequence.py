...
#	print self.subExposures
    ...
    def Restart(self):
        ...
        if self.nestedSubSequence is not None:
    ...
    def UpdateState(self):
        ...
        if self.aborted or ((self.nMisEvents > self.subMaxMissed) and not self.WLtype):
            self.state = SEQ_LOST
        elif ((self.nAllEvents >= self.subEvents) and not self.WLtype) \
                or ((self.nObsEvents >= self.subEvents) and self.WLtype):
    ...
    def RankTimeWindow(self, date):
        ...
        if self.nestedSubSequence is not None:
            if self.nestedSubSequence.IsIdle() is False:
    ...
    def ObserveEvent(self, date, obsHistID):
        ...
        if self.nestedSubSequence is not None:
            ...
            if self.nestedSubSequence.IsComplete() is False:
                return False
            else:
                self.nestedSubSequence.Restart()
        ...
        #print("fieldID=%d, subseq=%s ahistory=%s" % (self.field, self.subName, str(self.allHistory)))
    ...
    def MissEvent(self, date, misHistID):
        ...
        if self.nestedSubSequence is not None:
            ...
            if self.nestedSubSequence.IsLost() is False:
                return
...
class SuperSequence(LSSTObject):
    ...
    def IsIdle(self, name=None):
        ...
        if name is None:
            if self.state == SEQ_IDLE:
                ...
                return False
        elif self.masterSubSequence is None:
    ...
    def ObserveEvent(self, date, name, obsHistID):
        ...
        if self.subSequence[name].ObserveEvent(date, obsHistID):
            self.allHistory.append((date, name))
            self.obsHistory.append((date, name))

            if len(self.allHistory) == 1:
                self.date = date
                if self.masterSubSequence is not None:
                    for name in self.subSeqName:
    ...
    def RankTimeWindow(self, name, date):
        ...
        if not self.subSequence[name].HasTimeWindow():
            ...
            if self.progressToStartBoost < avgprogress < 1.0:
                seqneed += self.maxBoostToComplete * \
                    (avgprogress - self.progressToStartBoost) / (1.0 - self.progressToStartBoost)
            if avgprogress < 1.0:
