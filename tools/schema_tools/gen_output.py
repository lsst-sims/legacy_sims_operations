...
def check_columns_if_they_exist(hname, database, sessionID):
	columns = ("obsHistID", "sessionID", "fieldID", "fieldRA", "fieldDec", "filter", "expDate", "expMJD", "night", "visitTime", "visitExpTime", "finRank", "finSeeing", "transparency", "airmass", "vSkyBright", "filtSkyBright", "rotSkyPos", "lst", "altitude", "azimuth", "dist2Moon", "solarElong", "moonRA", "moonDec", "moonAlt", "moonAZ", "moonPhase", "sunAlt", "sunAz", "phaseAngle", "rScatter", "mieScatter", "moonIllum", "moonBright", "darkBright", "rawSeeing", "wind", "humidity", "slewDist", "slewTime", "fivesigma", "perry_skybrightness", "fivesigma_ps", "skybrightness_modified", "fivesigma_modified");
	simname = "output_%s_%d" % (hname, sessionID);    
...
def create_output_table(hname, database, sessionID):
	...
	sql = 'create table output_%s_%d (obsHistID int(10) unsigned not null, sessionID int(10) unsigned not null, propID int(10), fieldID int(10) unsigned not null, fieldRA double, fieldDec double, filter varchar(8), expDate int(10) unsigned, expMJD double, night int(10) unsigned, visitTime double, visitExpTime double, finRank double, finSeeing double, transparency double, airmass double, vSkyBright double, filtSkyBright double, rotSkyPos double, lst double, altitude double, azimuth double, dist2Moon double, solarElong double, moonRA double, moonDec double, moonAlt double, moonAZ double, moonPhase double, sunAlt double, sunAz double, phaseAngle double, rScatter double, mieScatter double, moonIllum double, moonBright double, darkBright double, rawSeeing double, wind double, humidity double, slewDist double, slewTime double, fivesigma double, perry_skybrightness double, fivesigma_ps double, skybrightness_modified double, fivesigma_modified double);' % (hname, sessionID)
	ret = getDbData(database, sql)
	sql = 'select obsHistID, Session_sessionID as sessionID, Field_fieldID as fieldID, filter, expDate, expMJD, night, visitTime, visitExpTime, finRank, finSeeing, transparency, airmass, vSkyBright, filtSkyBright, rotSkyPos, lst, alt as altitude, az as azimuth, dist2Moon, solarElong, moonRA, moonDec, moonAlt, moonAZ, moonPhase, sunAlt, sunAz, phaseAngle, rScatter, mieScatter, moonIllum, moonBright, darkBright, rawSeeing, wind, humidity from ObsHistory where Session_sessionID = %d;' % (sessionID)
	ret = getDbData(database, sql)
	...
	for k in range(len(ret)):
		...
		for i in range(len(prp)):
			...
			(moonAz_RAD,d1,d2,moonAlt_RAD,d4,d5,d6,d7,d8) = slalib.sla_altaz(moonha_RAD, moonDec_RAD, (ctioLat*DEG2RAD));		
			malt_RAD = float(ret[k][23]);
			mphase = float(ret[k][25]);
			saz = float(ret[k][27]);
			salt = float(ret[k][26]);
			mphase = math.acos((mphase/50)-1)*180/math.pi;
			skyB = etc.getSkyBrightnessAB(bandindex, MJD, extC, (alt_RAD * RAD2DEG), (az_RAD * RAD2DEG), (malt_RAD * RAD2DEG) , (moonAz_RAD * RAD2DEG), mphase, (saz * RAD2DEG), (salt * RAD2DEG));
			...
			m5_2 = Cm + 0.50*(skyB-21) + 2.5*math.log10(0.7/seeing) +1.25*math.log10(expTime/30) - kAtm*(airmass-1) + 1.1*tauCloud; # Using Perry's SkyBrightness
			...
			if (salt*RAD2DEG) > -18.0 and (filter == 'y' or filter == 'z'):
				...
				sql = 'insert into output_%s_%d (obsHistID, sessionID, propID, fieldID, fieldRA, fieldDec, filter, expDate, expMJD, night, visitTime, visitExpTime, finRank, finSeeing, transparency, airmass, vSkyBright, filtSkyBright, rotSkyPos, lst, altitude, azimuth, dist2Moon, solarElong, moonRA, moonDec, moonAlt, moonAZ, moonPhase, sunAlt, sunAz, phaseAngle, rScatter, mieScatter, moonIllum, moonBright, darkBright, rawSeeing, wind, humidity, slewDist, slewTime, fivesigma, perry_skybrightness, fivesigma_ps, skybrightness_modified, fivesigma_modified) values (%d, %d, %d, %d, %f, %f, "%s", %d, %f, %d, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f)' % (hname, sessionID, ret[k][0], ret[k][1], prp[i][0], ret[k][2], math.radians(fld[0][0]), math.radians(fld[0][1]), ret[k][3], ret[k][4], ret[k][5], ret[k][6], ret[k][7], ret[k][8], ret[k][9], ret[k][10], ret[k][11], ret[k][12], ret[k][13], ret[k][14], ret[k][15], ret[k][16], ret[k][17], ret[k][18], ret[k][19], ret[k][20], ret[k][21], ret[k][22], ret[k][23], ret[k][24], ret[k][25], ret[k][26], ret[k][27], ret[k][28], ret[k][29], ret[k][30], ret[k][31], ret[k][32], ret[k][33], ret[k][34], ret[k][35], ret[k][36], slw[0][1], slw[0][0], m5_1, skyB, m5_2, filtsky, m5_1);
			else :
				sql = 'insert into output_%s_%d (obsHistID, sessionID, propID, fieldID, fieldRA, fieldDec, filter, expDate, expMJD, night, visitTime, visitExpTime, finRank, finSeeing, transparency, airmass, vSkyBright, filtSkyBright, rotSkyPos, lst, altitude, azimuth, dist2Moon, solarElong, moonRA, moonDec, moonAlt, moonAZ, moonPhase, sunAlt, sunAz, phaseAngle, rScatter, mieScatter, moonIllum, moonBright, darkBright, rawSeeing, wind, humidity, slewDist, slewTime, fivesigma, perry_skybrightness, fivesigma_ps, skybrightness_modified, fivesigma_modified) values (%d, %d, %d, %d, %f, %f, "%s", %d, %f, %d, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f)' % (hname, sessionID, ret[k][0], ret[k][1], prp[i][0], ret[k][2], math.radians(fld[0][0]), math.radians(fld[0][1]), ret[k][3], ret[k][4], ret[k][5], ret[k][6], ret[k][7], ret[k][8], ret[k][9], ret[k][10], ret[k][11], ret[k][12], ret[k][13], ret[k][14], ret[k][15], ret[k][16], ret[k][17], ret[k][18], ret[k][19], ret[k][20], ret[k][21], ret[k][22], ret[k][23], ret[k][24], ret[k][25], ret[k][26], ret[k][27], ret[k][28], ret[k][29], ret[k][30], ret[k][31], ret[k][32], ret[k][33], ret[k][34], ret[k][35], ret[k][36], slw[0][1], slw[0][0], m5_1, skyB, m5_2, skyB, m5_2);
			#print sql
