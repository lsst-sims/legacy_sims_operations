...
def report(hostname, sessionID, notcs):

    print sepline

    label = 'session info'
    sql = 'select sessionHost, sessionDate from tSession_%s_%d where sessionID=%d' % (hostname, sessionID, sessionID)
    ...
    label = 'config'
    sql   = 'select paramName,paramValue from tConfig_%s_%d where Session_sessionID=%d and moduleName="LSST" order by paramIndex' % (hostname, sessionID, sessionID)
...
#    print dict_LSST
    ...
    sql   = 'select count(event) from tTimeHistory_%s_%d where Session_sessionID=%d and event=%d' % (hostname, sessionID, sessionID, START_NIGHT)
    ret   = getDbData(sql, label)
    count_nights = ret[0][0]
    print "%s: %s" % (label, count_nights)

    label = 'number of visits'
    sql   = 'select count(*) from tSlewHistory_%s_%d where ObsHistory_Session_sessionID=%d' % (hostname, sessionID, sessionID)
    ret   = getDbData(sql, label)
    ...
    sql   = 'select avg(visitExpTime) from tObsHistory_%s_%d where Session_sessionID=%d' % (hostname, sessionID, sessionID)
    ret   = getDbData(sql, label)
    avg_visittime = ret[0][0]
    print "%s: %5.2fs" % (label, avg_visittime)

    label = 'average visit time'
    sql   = 'select avg(visitTime) from tObsHistory_%s_%d where Session_sessionID=%d' % (hostname, sessionID, sessionID)
    ret   = getDbData(sql, label)
    avg_visittime = ret[0][0]
    print "%s: %5.2fs [TBD]" % (label, avg_visittime)

    label = 'average slew time'
    sql   = 'select avg(slewTime) from tSlewHistory_%s_%d where ObsHistory_Session_sessionID=%d' % (hostname, sessionID, sessionID)
    ret   = getDbData(sql, label)
    ...
    reportAngleStats('telAlt', hostname, sessionID)
    reportAngleStats('telAz' , hostname, sessionID)
    reportAngleStats('rotTelPos', hostname, sessionID)
    
    reportActivity('DomAlt'     , hostname, sessionID, count_visits)
    reportActivity('DomAz'      , hostname, sessionID, count_visits)
    reportActivity('TelAlt'     , hostname, sessionID, count_visits)
    reportActivity('TelAz'      , hostname, sessionID, count_visits)
    reportActivity('Rotator'    , hostname, sessionID, count_visits)
    reportActivity('Filter'     , hostname, sessionID, count_visits)
    reportActivity('TelOpticsOL', hostname, sessionID, count_visits)
    reportActivity('Readout'    , hostname, sessionID, count_visits)
    reportActivity('Settle'     , hostname, sessionID, count_visits)
    reportActivity('DomSettle'  , hostname, sessionID, count_visits)
    reportActivity('TelOpticsCL', hostname, sessionID, count_visits)
    
    reportMaxSpeed('DomAlt', hostname, sessionID)
    reportMaxSpeed('DomAz' , hostname, sessionID)
    reportMaxSpeed('TelAlt', hostname, sessionID)
    reportMaxSpeed('TelAz' , hostname, sessionID)
    reportMaxSpeed('Rot'   , hostname, sessionID)

    print sepline

    label = 'list of proposals'
    sql   = 'select propID,propConf,propName from tProposal_%s_%d where Session_sessionID=%d' % (hostname, sessionID, sessionID)
    ret   = getDbData(sql, label)
    list_proposals = ret

    for filter in ['u','g','r','i','z','y']:
	label = 'visits count for filter %s' % (filter)
	sql   = 'select count(*) from tObsHistory_%s_%d where Session_sessionID=%d and filter="%s"' % (hostname, sessionID, sessionID, filter)
	ret   = getDbData(sql, label)
	...
	sql   = 'select count(*) from tObsHistory_Proposal_%s_%d where Proposal_propID=%d' % (hostname, sessionID, propID)
	ret   = getDbData(sql, label)
	numVisits = ret[0][0]
	...
	for filter in ['u','g','r','i','z','y']:
	    ...
	    sql   = 'select count(*) from tObsHistory_%s_%d oh INNER JOIN tObsHistory_Proposal_%s_%d ohp ON oh.obsHistID=ohp.ObsHistory_obsHistID where ohp.Proposal_propID=%d and oh.filter="%s" and oh.Session_sessionID=%d' % (hostname, sessionID, hostname, sessionID, propID, filter, sessionID)
	    ret   = getDbData(sql, label)
...
def reportAngleStats(name, hostname, sessionID):

    label = 'statistics for angle %9s' % (name)
    sql   = 'select min(%s),max(%s),avg(%s),std(%s) from tSlewState_%s_%d join tSlewHistory_%s_%d where tSlewState_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d and state=1' % (name, name, name, name, hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID)
    ret   = getDbData(sql, label)
    print "%s: min=%6.1fd max=%6.1fd avg=%6.1fd std=%5.1fd" % (label, ret[0][0]*RAD2DEG, ret[0][1]*RAD2DEG, ret[0][2]*RAD2DEG, ret[0][3]*RAD2DEG)
    
def reportActivity(name, hostname, sessionID, total):

    label = 'slew activity for %12s' % (name)
    sql   = 'select avg(actDelay),max(actDelay),count(actDelay) from tSlewActivities_%s_%d join tSlewHistory_%s_%d where tSlewActivities_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d and activity="%s" and actDelay>0' % (hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID, name)
    ret   = getDbData(sql, label)
    A     = ret[0][0]
    M     = ret[0][1]
    N     = ret[0][2]
    sql   = 'select count(*),avg(actDelay) from tSlewActivities_%s_%d join tSlewHistory_%s_%d where tSlewActivities_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d and activity="%s" and inCriticalPath="True" and actDelay>0' % (hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID, name)
    ret   = getDbData(sql, label)
    C     = ret[0][0]
    CA    = ret[0][1]
    if CA == None:
...
def reportMaxSpeed(name, hostname, sessionID):

    label = 'slew maximum speed for %8s' % (name)
    sql   = 'select avg(abs(%sSpd)),max(abs(%sSpd)),max(slewCount) from tSlewMaxSpeeds_%s_%d join tSlewHistory_%s_%d where tSlewMaxSpeeds_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d' % (name, name, hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID)
    ret   = getDbData(sql, label)
    A     = ret[0][0]
    M     = ret[0][1]
    T     = ret[0][2]
    sql   = 'select count(*) from tSlewMaxSpeeds_%s_%d join tSlewHistory_%s_%d where tSlewMaxSpeeds_%s_%d.SlewHistory_slewID=tSlewHistory_%s_%d.slewID and ObsHistory_Session_sessionID=%d and abs(%sSpd)>=%f' % (hostname, sessionID, hostname, sessionID, hostname, sessionID, hostname, sessionID, sessionID, name, 0.99*M)
    ret   = getDbData(sql, label)
    C     = ret[0][0]
...
def reportSequences(label, propID):
    ...
    sql = 'select count(*) from tSeqHistory_%s_%d where sessionID=%d and propID=%d and endStatus=0' % (hostname, sessionID, sessionID, propID)
    ret   = getDbData(sql, label)
    N_complete = ret[0][0]
                                                                                                                                                                                                                    
    sql = 'select count(*) from tSeqHistory_%s_%d where sessionID=%d and propID=%d and endStatus=1' % (hostname, sessionID, sessionID, propID)
    ret   = getDbData(sql, label)
    N_lost_missed = ret[0][0]
                                                                                                                                                                                                                    
    sql = 'select count(*) from tSeqHistory_%s_%d where sessionID=%d and propID=%d and endStatus=3 and completion>0' % (hostname, sessionID, sessionID, propID)
    ret   = getDbData(sql, label)
    ...
    sql = 'select completion,count(0) from tSeqHistory_%s_%d where sessionID=%d and propID=%d group by completion' % (hostname, sessionID, sessionID, propID)
    ret   = getDbData(sql, label)
...
def reportWL(hostname, sessionID):

    label = 'ids_WL'
    sql   = 'select propID,propConf from tProposal_%s_%d where Session_sessionID=%d and propName="WL"' % (hostname, sessionID, sessionID)
	...
	propConf = ids_WL[q][1]
	label = 'Weak Lensing'
        sql   = 'select paramName,paramValue from tConfig_%s_%d where Session_sessionID=%d and moduleName="weakLensing" and nonPropID=%d order by paramIndex' % (hostname, sessionID, sessionID,propID)
        ...
        sql   = 'select count(Field_fieldID) from tProposal_Field_%s_%d where Proposal_propID=%d;' % (hostname, sessionID, propID)
        ret = getDbData(sql, label)
...
#	f5Visits = eval(dict_WL[str(f5)+'FilterVisits'])
	...
	sql = 'create table tmpObs select propID,fieldID,fieldRA,fieldDec,filter,expDate from tObsHistory_%s_%d where sessionID=%d and propID=%d group by expDate order by propID,fieldID,filter' % (hostname, sessionID, sessionID, propID)
	ret = getDbData(sql, label)
...
def reportNEA(sessionID):
    ...
    sql   = 'select propID,propConf from tProposal where Session_sessionID=%d and propName="NEA"' % (sessionID)
    ret   = getDbData(sql, label)
    if ret == None:
        return
	...
	sql = 'select count(*) from tSeqHistory where Proposal_propID=%d and endStatus=2' % (propID)
	ret   = getDbData(sql, label)
...
def reportSN(sessionID):
    ...
    label = 'Super Nova'
    sql   = 'select propID,propConf from tProposal where Session_sessionID=%d and propName="SN"' % (sessionID)
    ret   = getDbData(sql, label)
    if ret == None:
        return
...
def reportSNSS(sessionID):
    ...
    sql   = 'select propID,propConf from tProposal where Session_sessionID=%d and propName="SNSS"' % (sessionID)
    ret   = getDbData(sql, label)
    if ret == None:
        return
    if len(ret) == 0:
...
def reportKBO(sessionID):
    ...
    sql   = 'select propID,propConf from tProposal where Session_sessionID=%d and propName="KBO"' % (sessionID)
    ret   = getDbData(sql, label)
    if ret == None:
        return
...
def reportWLTSS(sessionID, host):
    ...
    sql   = 'select propID,propConf from tProposal where Session_sessionID=%d and propName="WLTSS"' % (sessionID)
	...
	sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqFilters" order by paramIndex' % (sessionID, propID)
	list_filter = getDbData(sql, label)

        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqEvents" order by paramIndex' % (sessionID, propID)
        list_events = getDbData(sql, label)

        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqInterval" order by paramIndex' % (sessionID, propID)
        list_interval = getDbData(sql, label)
                                                                                                                 
        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqWindowStart" order by paramIndex' % (sessionID, propID)
        list_windowstart = getDbData(sql, label)
                                                                                                                 
        sql = 'select paramValue from tConfig where Session_sessionID=%d and nonpropID=%d and paramName="SubSeqWindowEnd" order by paramIndex' % (sessionID, propID)
...
try:
    ...
    print "..........Use python summaryreport.py <sessionID>\n\n"
    done 
try:
    option1 = val[1]
    if option1 == "notcs":
    ...
    notcs = False
report(host, int(sessionID), notcs)
