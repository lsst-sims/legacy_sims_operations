...
def parseArgs():
    ...
    #opsim_version = check_output(["opsim-version.sh"])
    #opsim_version = opsim_version.strip(' \t\n\r')
    default_opsim_run_dir = os.path.join(os.path.expanduser("~"), "opsim-run")#, opsim_version)

    parser = argparse.ArgumentParser(
            description='''opsim configuration tool. Creates an execution
directory (opsim_run_dir) which will contains configuration and execution
data for a given opsim instance. Deploys values from meta-config file $opsim_run_dir/opsim-meta.conf
in all opsim configuration files and databases. Default behaviour will configure a mono-node
instance in ''' + default_opsim_run_dir + '''. IMPORTANT : --all MUST BE USED
FOR A  SETUP FROM SCRATCH.''',
    ...
    default_meta_config_file = os.path.join(args.opsim_run_dir, "opsim-meta.conf")
    ...
    if configure.PREPARE in args.step_list:
        ...
        if os.path.exists(args.opsim_run_dir):
            if args.force or configure.user_yes_no_query(
                ...
                logging.info("Stopping opsim configuration, please specify an other configuration directory")
                sys.exit(1)
