...
def read_config(config_file):
    ...
    config['opsim']['scratch_dir'] = os.path.join( "/dev", "shm", "opsim-%s-%s" %
                                        (getpass.getuser(),
                                        hashlib.sha224(config['opsim']['run_base_dir']).hexdigest())
                                    )
...
def restart(service_name):
        ...
        initd_path = os.path.join(config['opsim']['run_base_dir'],'etc','init.d')
