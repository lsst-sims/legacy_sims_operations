...
#            (PathVariable('LOG_DIR', 'log install dir', _findPrefixFromName('LOG'), PathVariable.PathIsDir)),
            (PathVariable('python_relative_prefix', 'opsim install directory for python modules, relative to prefix', os.path.join("lib", "python"), PathVariable.PathIsDirCreate))
            )
    opts.Update(env)
...
#            (PathVariable('LOG_LIB', 'log libraries path', os.path.join(env['LOG_DIR'], "lib"), PathVariable.PathIsDir)),
    ...
    opts.AddVariables(
            (PathVariable('python_prefix', 'opsim install directory for python modules', os.path.join(env['prefix'], env['python_relative_prefix']), PathVariable.PathIsDirCreate))
	    )
    opts.Update(env)
...
def init(src_dir):
    ...
    opts.AddVariables(
            (PathVariable('build_dir', 'OpSim build dir', os.path.join(src_dir, 'build'), PathVariable.PathIsDirCreate)),
    ...
    opts.AddVariables(
            (PathVariable('prefix', 'opsim install dir', src_dir, PathVariable.PathIsDirCreate))
    )
    opts.Update(env)
