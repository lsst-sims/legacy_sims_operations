...
# This includes the primary Environment object (state.env), the message log
# (state.log), the command-line variables object (state.opts), and a dictionary
# of command-line targets used to setup aliases, default targets, and
# dependencies (state.targets).  All four of these variables are aliased to the
# main lsst.sconsUtils scope, so there should be no need for users to deal with
# the state module directly.
#
# These are all initialized when the module is imported, but may be modified by
# other code (particularly dependencies.configure()).
##
...
#  BasicSConstruct.finish(). While one can still use env.Alias to setup aliases
#  (and should for "install"), putting targets here will generally provide
#  better build-time dependency handling (like ensuring everything is built
#  before we try to install, and making sure SCons doesn't rebuild the world
#  before installing).
#
#  Users can add additional keys to the dictionary if desired.
#
#  Targets should be added by calling extend() or using += on the dict values,
#  to keep the lists of targets from turning into lists-of-lists.
##
...
def _findPrefixFromName(product):
    ...
    if not prefix:
        log.fail("Could not locate %s install prefix using %s" %
...
def _getBinPathFromBinList(binList, msg=None):
    ...
    while i < len(binList) and not binFullPath:
        ...
        i = i + 1
    if not binFullPath:
        raise SCons.Errors.StopError('Could not locate at least one binary in'
                                     ' : %s' % binList)
    else:
        return binFullPath
...
def _findPrefixFromPath(key, binFullPath):
    ...
    if not prefix:
        log.fail("Could not locate install prefix for product containing "
                 "next binary : " % binFullPath)
    return prefix

def _initOptions():
    SCons.Script.AddOption('--verbose', dest='verbose', action='store_true',
                           default=False,
                           help="Print additional messages for debugging.")
    SCons.Script.AddOption('--traceback', dest='traceback',
                           action='store_true', default=False,
                           help="Print full exception tracebacks when errors"
                           " occur.")
...
def _saveState():
    """
    Save state such as optimization level used.  The scons mailing lists were
    unable to tell RHL how to get this back from .sconsign.dblite
    """
    ...
    try:
        confFile = os.path.join(os.path.join(env['prefix'], "admin"),
...
def init(src_dir):
    ...
    opts.AddVariables((PathVariable('build_dir', 'OpSim build dir',
    ...
    opts.AddVariables((PathVariable('prefix', 'opsim install dir', src_dir,
                                    PathVariable.PathIsDirCreate)))
    opts.Update(env)
