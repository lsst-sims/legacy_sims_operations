...
def install_python_module(env, target, source):
    ...
    for f in source_lst:
        target = fileutils.replace_base_path(source_dir_path,
                                             python_path_prefix, f, env)
        state.log.debug("install_python_module() : source %s, target %s" %
                        (f, target))
...
def clean_python_path_dir(target_lst, env):
    ...
    # take common prefix of empty_py_dirs and check if upper dir to python_path
    # are empty
    # Keep __init__.py file if a subdirectory contains at least one file of
    # bottom_up_target_list
